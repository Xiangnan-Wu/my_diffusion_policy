...
class RealPushTImageDataset(BaseImageDataset):
    def __init__(
        ...
        if use_cache:
            ...
            )  # 把shape_meta:OmegaConf 对象转化为字典str
...
def _get_replay_buffer(dataset_path, shape_meta, store):
    ...
    lowdim_shapes = dict()
    obs_shape_meta = shape_meta[
        "obs"
    ]  # camera_1 + shape   camera_3 + shape    robot_eef_pose + shape
    for key, attr in obs_shape_meta.items():
...
def test():
    ...
    with hydra.initialize("../config"):
        cfg = hydra.compose(
            "train_robomimic_real_image_workspace"
        )  # 合成配置 train_robomimic_real_image_workspace 中包含 defaults 中的 task/real_pusht_image.yaml 将其转化为一个整体配置文件
        OmegaConf.resolve(
            cfg
        )  # 解析配置中的差值表达式 即 ${eval:'${n_obs_steps}-1+${n_latency_steps}'}等
        dataset = hydra.utils.instantiate(
            cfg.task.dataset
        )  # 实例化 dataset._target_ 指向的类 这里是 diffusion_policy.dataset.real_pusht_image_dataset.RealPushTImageDataset
